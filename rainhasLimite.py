...
def executar_tempera_simulada():
    ...
    while len(solucoes) < max_solucoes and total_iteracoes < max_execucoes_gerais:
        ...
        if calcular_fx(solucao) == 28:
            print(f"Solução perfeita encontrada: {solucao}, f(x): {calcular_fx(solucao)}")
            solucoes.append(solucao)
            tempos_de_execucao.append(time.time() - start_time)
            break  #para aexecução 
    ...
    # gráfico de tempo do total da execução  (nao ta funcionando mt bem não esse aqui)
